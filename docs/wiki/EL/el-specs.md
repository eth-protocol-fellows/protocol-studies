...
|--------------|-------------------------|-------------------------------------------------------------------|
| Type 0 / 1   | `gasLimit`              | Max gas the transaction can consume                               |
| Type 0 / 1   | `gasPrice`              | Full gas price paid to the proposer                               |
| Type 2       | `gasLimit`              | Max gas the transaction can consume                               |
...
| Tx Type    | Effective Gas Price                                                 | Actual Cost                       | Distribution                           |
|------------|----------------------------------------------------------------------|------------------------------------|----------------------------------------|
| Type 0 / 1 | `gasPrice`                                                           | `gasUsed × gasPrice`              | Entire amount paid to proposer         |
| Type 2     | `baseFee + min(maxPriorityFeePerGas, maxFeePerGas − baseFee)`       | `gasUsed × effectiveGasPrice`     | `baseFee` burned, tip to proposer      |

Note that the type 2 transaction effective gas price is capped to ensure the total paid per gas unit never exceeds `maxFeePerGas`. If the `baseFee` is high, the actual tip (`maxPriorityFeePerGas`) may be reduced to fit within this total budget.
