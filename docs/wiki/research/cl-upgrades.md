...
This page summarizes proposed and recent consensus-layer upgrade areas that are relevant for protocol contributors. Some of these items are active research or draft EIPs, so they should be read as roadmap directions rather than finalized mainnet behavior unless otherwise noted.

## Proposer-Builder Separation

Proposer-builder separation (PBS) separates the role of choosing the best execution payload from the role of proposing the beacon block. In today's out-of-protocol PBS design, validators commonly use MEV-Boost to outsource block construction to builders through relays. This lets specialized builders compete to construct high-value execution payloads while validators continue to perform consensus duties.

PBS is important because block production is affected by MEV, censorship resistance, latency, and validator operational complexity. Separating proposers and builders can reduce the need for every validator to run sophisticated block-building infrastructure, but it also introduces additional trust and centralization concerns around builders and relays.

## Enshrined PBS

Enshrined PBS (ePBS) is a proposed direction for moving parts of proposer-builder separation into the Ethereum protocol itself. EIP-7732 is a draft proposal that introduces consensus-layer mechanisms for separating consensus block proposal from execution payload construction.

The motivation for ePBS is to reduce reliance on trusted relays and make the proposer-builder interaction more protocol-native. It also aims to improve the timing constraints on validators: proposers should be able to make consensus-layer progress without having to fully validate a large execution payload before proposing their beacon block.
...
At a high level, FOCIL aims to make censorship more difficult by ensuring that valid transactions seen by committee members cannot be ignored indefinitely by block builders or proposers. This is especially relevant in a PBS or ePBS world, where execution payload construction may be concentrated among specialized builders.
...
EIP-7251 proposes increasing the maximum effective balance of validators. This EIP is Final and is part of the Pectra upgrade.

Today, validators have historically had an effective balance capped at 32 ETH. This design creates a large number of validator entries when stake is distributed across many 32 ETH validators. Increasing the maximum effective balance allows larger effective balances per validator and enables validator consolidation.
